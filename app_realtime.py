...
REFRESH_RATE = 2  # QUAN TRỌNG: Để 2s để thấy biểu đồ trôi
...
def get_recent_data(limit=500): # Tăng limit để đường vẽ dài hơn
    try:
        ...
        if not df.empty:
            ...
            # Lấy dữ liệu 12 tiếng gần nhất để đảm bảo liền mạch
...
# --- ĐÂY LÀ PHẦN SỬA QUAN TRỌNG: BIỂU ĐỒ TRÔI (SLIDING WINDOW) ---
def create_trend_chart(df, dev_name):
    # Tính giờ hiện tại theo múi giờ VN để làm mốc trôi
    now_vn = datetime.utcnow() + timedelta(hours=7)
    # Khung nhìn: 15 phút trước -> Hiện tại
    x_range = [now_vn - timedelta(minutes=15), now_vn] 

    fig = go.Figure()
    
    if not df.empty:
        # Chỉ vẽ những điểm nằm trong hoặc gần khung nhìn để tối ưu
        mask = (df['time'] >= (x_range[0] - timedelta(minutes=1))) & (df['time'] <= x_range[1])
        plot_df = df.loc[mask]
        
        # Nếu plot_df rỗng (do chưa có data mới), lấy 1 điểm cuối cùng để tránh lỗi
        if plot_df.empty and not df.empty:
             plot_df = df.head(1)

        fig.add_trace(go.Scatter(
            x=plot_df['time'], y=plot_df['Speed'],
            fill='tozeroy', mode='lines', # Bỏ marker để nhìn mượt hơn
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            range=x_range,    # <--- KHÓA TRỤC X ĐỂ NÓ TRÔI THEO THỜI GIAN
...
def render_realtime_tab():
    ...
    st.caption(f"Last update: {now_str} | Auto-scroll: ON")
...
# ===============================================================
# 4. REPORT TAB (GIỮ NGUYÊN CODE GỐC CỦA BRO)
# ===============================================================
