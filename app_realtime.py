...
REFRESH_RATE = 2  # TĂNG TỐC ĐỘ REFRESH LÊN 2 GIÂY ĐỂ THẤY BIỂU ĐỒ TRÔI
...
def get_recent_data(limit=500): # TĂNG LIMIT LÊN 500 ĐỂ LẤY NHIỀU ĐIỂM HƠN (DÀY ĐẶC HƠN)
    try:
        ...
        if not df.empty:
            ...
            # Lấy dữ liệu 12h gần nhất để đảm bảo liền mạch
...
# --- BIỂU ĐỒ TRÔI (SLIDING WINDOW) - CHÌA KHÓA CỦA SỰ MƯỢT MÀ ---
def create_trend_chart(df, dev_name):
    # Kỹ thuật: Luôn hiển thị khung 15 phút tính từ THỜI ĐIỂM HIỆN TẠI (NOW)
    # Vì NOW luôn tăng -> Khung nhìn luôn trôi sang phải -> Tạo cảm giác Real-time
    now = datetime.now()
    ...
    if not df.empty:
        fig.add_trace(go.Scatter(
            x=df['time'], y=df['Speed'], fill='tozeroy', mode='lines', # Bỏ markers để nhìn mượt hơn khi dày đặc
    ...
    fig.update_layout(
        title=dict(text="Diễn biến 15 phút qua (Live Scroll)", font=dict(size=14, color="#555")),
        height=250, margin=dict(l=10, r=10, t=40, b=10),
        xaxis=dict(
            showgrid=False, tickformat='%H:%M:%S',
            range=x_range,    # <--- KHÓA TRỤC X VÀO KHUNG GIỜ HIỆN TẠI
            fixedrange=True   # <--- KHÔNG CHO ZOOM ĐỂ GIỮ HIỆU ỨNG TRÔI
        ),
...
def render_realtime_tab():
    ...
    def update_loop():
        df_all = get_recent_data(500) # Lấy 500 điểm để đảm bảo đường vẽ dài và liền mạch
        ...
        if df_all.empty:
            for dev in DEVICES:
                with cols_map[dev]: st.warning("⏳ Đang chờ dữ liệu...")
            return
        ...
        for dev in DEVICES:
            ...
            # RENDER GIAO DIỆN CÔNG NGHIỆP (GIỮ NGUYÊN)
            with current_col:
                with st.container(border=True):
                    ...
                    # Key cố định để không bị nháy, nhưng nội dung biểu đồ sẽ thay đổi theo trục thời gian
